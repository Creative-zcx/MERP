...
class PerceiverIO(nn.Module):
    ...
    def __init__(
        ...
        self._multi_preprocessor = MultimodalPreprocessor(input_preprocessors=input_preprocessors,
                                                          ...
                                                          input_channels=input_channels,is_gate=is_gated,is_skip_connection=is_skip_connection)
    ...
    def forward(self, inputs, *, subsampled_output_points=None,
        ...
        cross_attn_out = self._encoder.cross_attend(
            encoder_query, 
            inputs
        )
        ...
        if return_intermediates:
            return {
                'cross_attention': cross_attn_out,  # [64, 6, d2]
                'latents': latents,                 # [64, 6, d2]
                'outputs': outputs                  # [64, 6, d1]
            }
...
class MultimodalPreprocessor(nn.Module):
  ...
  """
    def __init__(self, input_preprocessors, mask_probs, min_padding_size, input_channels, is_gate=False, is_skip_connection=True):
        """Constructor.
    Args:
      input_preprocessors: dict mapping modality name to preprocessor
      mask_probs: dict mapping modality name to masking probability of that modality
      ...
      is_gate: 是否在多模态特征融合时使用门控单元和门控残差连接。Default: False.
      is_skip_connection: 是否使用门控残差连接。Default: True.
    """
        super().__init__()
        ...
        # 如果使用门控模块，则为每个模态构造gate+transform层（轻量化实现：单个线性层加sigmoid）
        if self.is_gate:
            self.gate_transform_layers = nn.ModuleDict()
            # 假设每个模态输入经过padding后的通道数均为 self._common_channels
            for modality in self._preprocessors.keys():
                self.gate_transform_layers[modality] = nn.Linear(self._common_channels, self._common_channels)
    ...
    def forward(self, inputs: torch.Tensor, *, pos: Optional[torch.Tensor] = None) -> PreprocessorOutputT:
        if self._preprocessors is None:
            ...
            for modality, preprocessor in self._preprocessors.items():
                outputs[modality], inputs_without_pos[modality] = preprocessor(inputs[modality], pos=pos)
                if modality == 'label':
                    # 处理标签数据
                    outputs[modality] = torch.squeeze(outputs[modality])
        ...
        if self.padding_embeddings is not None:
            ...
            for modality, output in outputs.items():
                ...
                padding = torch.broadcast_to(
                    pos_enc,
                    [output.shape[0], output.shape[1], self._common_channels - output.shape[2]])
                output_padded = torch.cat([output, padding], dim=2)
                if self.is_gate:
                    candidate = self.gate_transform_layers[modality](output_padded)
                    gate = torch.sigmoid(candidate)
                    if self.is_skip_connection:
                        output_padded = output_padded + gate * (candidate - output_padded)
                    else:
                        output_padded = gate * candidate
        ...
        if self._mask_probs is not None and self.training:
            masked = {}
            for modality, output in outputs.items():
                ...
                mask_prob = mask_prob - 0.01 if mask_prob >= 1.0  else mask_prob
        ...
        return (torch.cat(outputs, dim=1), modality_sizes, inputs_without_pos)
