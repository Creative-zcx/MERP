...
class FeaturePreprocessor(nn.Module):
    """Preprocessor for pre-extracted features with position encoding.
    Args:
        input_channels (int): Number of input feature channels
        seq_len (int): Length of input sequence
        position_encoding_type (PosEncodingType): Type of position encoding
        concat_or_add_pos (str): Whether to concatenate or add position encodings
        **position_encoding_kwargs: Additional keyword arguments for position encoding
    """
    def __init__(
        self,
        input_channels: int,
        ...
        **position_encoding_kwargs
        ...
        # Modify position encoding construction
        self._positional_encoding = position_encoding.build_position_encoding(
            position_encoding_type=position_encoding_type,
            index_dims=[seq_len],
            **position_encoding_kwargs,
    ...
    def forward(self, inputs: torch.Tensor, *,
        ...
        # Combine features and position encoding
        if self._concat_or_add_pos == "concat":
            inputs_with_pos = torch.cat([inputs, pos_enc], dim=-1)
        else:  # "add"
            inputs_with_pos = inputs + pos_enc
